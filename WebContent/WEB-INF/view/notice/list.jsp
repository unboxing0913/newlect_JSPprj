		...
		<main class="main">
			...
			<div class="notice margin-top">
				...
				<table class="table">
					...
					//전체를 돌면서 가져와야하기때문에 list객체 생성해서 request를 받아준다.
					//태그라이브러리를 사용하지 않았을때 반복문을 돌리기위함
					for(Notice n : list){		
					pageContext.setAttribute("n", n);
					//EL문은 저장소에서 가져오는 것 이기떄문에 따로 Attribute 처리를 해줘야 사용할수있다.
					%> --%>
					
				 	<c:forEach var="n" items="${list}">
					<tr>
						<td>${n.id}</td>
						<td class="title indent text-align-left"><a href="detail?id=${n.id }">${n.title }<span style="color:orange;">[${ n.cmtCount}]</span></a></td>
						                                                    <!-- jsp가아닌 서블릿주소로 요청 -->   
						<td>${n.writerId }</td>
